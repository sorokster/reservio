...
class ReservationsService extends ApiService {
  ...
  /**
   * Cancel a reservation (update status to 'canceled' instead of deleting)
   */
  async cancelReservation(id: number | string): Promise<Reservation> {
    // Update reservation status to 'canceled' instead of deleting
    return this.patch<Reservation>(API_ENDPOINTS.RESERVATIONS.UPDATE(id), {
      statuses_data: [{ status: 'canceled' }]
    });
  }
